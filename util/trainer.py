...
class Trainer:
    """
    Multi-signal trainer / evaluator with a minimal infer() mode.

    Expected dataloader batch format:
        inputs, targets_list, snrs, fps

    where:
        inputs:       Tensor [B, H, W]
        targets_list: List[List[dict]]
        snrs:         Tensor/List
        fps:          List[str]
    """
    ...
    def _extract_groups(self, yolo_boxes, spec):
        ...
        # fallback for old single-signal preprocess
    ...
    def _match_groups_to_targets(self, groups, targets, spec_shape):
        ...
        Returns:
            matched: list of dicts with keys:
                boxes, group_idx, target_idx, label, target, group
    ...
    def _build_matched_instances(
        ...
        images = torch.stack(batch_images, dim=0)     # [N,1,H,W]
    ...
    def _compute_image_exact_acc(self, metas, labels, preds, image_infos):
        """
        image exact acc:
            a source image is counted as correct iff all matched instances from that image are correct
            and num_matched == num_targets
        """
    ...
    def _save_confusion_matrix(self, y_true, y_pred, split_name="val", epoch: Optional[int] = None):
        ...
        label_ids = sorted(self.inv_class_map.keys()) if len(self.inv_class_map) > 0 else sorted(set(y_true) | set(y_pred))
        if len(label_ids) == 0:
            return
    ...
    # ------------------------- evaluation -------------------------
    @torch.no_grad()
    def evaluate_loader(self, loader, epoch=0, split_name="val", save_confusion=True, save_summary=True):
        ...
        for batch_idx, batch in enumerate(pbar):
            ...
            need_save_detect_result = (
                self.save_detect_vis_once and
                (not self._detect_vis_saved) and
                batch_idx == 0
            )
            ...
            images, labels, metas, batch_matched, batch_targets, image_infos = self._build_matched_instances(
                ...
                targets_list,
                sample_fps=fps,
                save_detect_result=need_save_detect_result or (split_name == "infer"),
            ...
            for meta, gt, pd in zip(metas, labels_cpu.tolist(), preds_cpu.tolist()):
        ...
        if save_summary:
            ...
            with open(save_dir / f"{stem}_instances.csv", "w", newline="", encoding="utf-8") as f:
                writer = csv.DictWriter(
                    f,
                    fieldnames=["file", "target_idx", "group_idx", "gt_label", "gt_name", "pred_label", "pred_name", "correct"]
                )
                writer.writeheader()
    ...
    def infer(self, infer_loader):
        ...
        self.logger.info(
            f"[Infer] loss={infer_loss:.4f}, "
            f"acc={infer_acc:.2f}, "
            f"match={infer_match_recall:.3f}, "
            f"img_exact={infer_image_exact_acc:.2f}"
        )
    ...
    def train(self):
        ...
        for epoch in range(self.epochs):
            ...
            self.logger.info(
                ...
                f"match={train_match_recall:.3f}, img_exact={train_image_exact_acc:.2f}"
            )
            self.logger.info(
                f"[Val]   loss={val_loss:.4f}, acc={val_acc:.2f}, "
                f"match={val_match_recall:.3f}, img_exact={val_image_exact_acc:.2f}"
            )
